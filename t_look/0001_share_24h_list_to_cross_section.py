...
# px
# we set days to AKI point distances as px values
# if non-procedure , we set NaN
...
with open(map_file_path, 'rb') as map_f:
    map_data = ['encounter_id'] + pickle.load(map_f)


# create shared_manager list for map_data
s_map_data = Manager().list(map_data)
del map_data
...
for year in range(2010, 2018 + 1):
    ...
    # load nested list data
    list_f = open(string_list_file_path, 'rb')
    list_data = pickle.load(list_f)
    list_f.close()

    # indexes of samples needed
    # idx_samples = np.r_[0:5, len(list_data) - 5: len(list_data)]
    # idx_samples = range(5)
    idx_samples = range(len(list_data))

    # init a variable to save all eligible cross-section sample
    all_sample = np.zeros((len(idx_samples), len(s_map_data)))

    # create shared_memory for all_sample
    ...
    # init result (have not abandon < 24h samples)
