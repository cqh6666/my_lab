...
for iteration_idx in range(cur_iteration, cur_iteration + 1):
    ...
    train_rank_x = last_x.loc[n_personal_model_each_iteration:, :].copy()
    train_rank_x.reset_index(drop=True, inplace=True)
    len_split = int(train_rank_x.shape[0] * select_rate)

    train_rank_y = last_y.loc[n_personal_model_each_iteration:].copy()
    ...
    table.to_csv('/panfs/pfs.local/work/liu/xzhang_sta/tangxizhuo/pg/0008_01_001_005-1_{}.csv'.format(iteration_idx), index=False)
