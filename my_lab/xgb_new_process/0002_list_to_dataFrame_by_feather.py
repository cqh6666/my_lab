...
def get_demo(idx, demo_input_data, map_data, all_sample):
    """
    Age_Hispanic_Race_Sex
    :param idx:
    :param demo_input_data:
    :param map_data:
    :param all_sample:
    :return:
    """
    Demo = np.array(demo_input_data)
...
def get_vital(idx, vital_input_data, input_t_time, map_data, all_sample):
    """
    Height_Weight_BMI_Smoking_Tabacco_TabaccoType_SBP_DBP
    :param idx:
    :param vital_input_data:
    :param input_t_time:
    :param map_data:
    :param all_sample:
    :return:
    """
    for m in range(len(vital_input_data)):
        try:
            temp = vital_input_data[m]  # [ [a, b] [c, d] ]
            temp1 = np.asarray(temp)
            temp2 = temp1[:, -1]  # [b, d]
            temp2 = list(map(float, temp2))  # str to float
            temp3 = [x for x in temp2 if x <= input_t_time]  # 找到记录时间点小于等于预测时间的值
            temp4 = np.max(temp3)  # 距离预测最近的记录点时间
            temp2.reverse()
            temp5 = temp2.index(temp4)  # 根据记录点时间找到对应的索引
            temp5 = len(temp2) - 1 - temp5

            if m == 3 or m == 4 or m == 5:
                """
                Smoking_Tabacco_TabaccoType
                ...
                indexNum = map_data.index(vitalIndex)
                all_sample[idx, indexNum] = temp[temp5][0]  # get value
        except:
            continue
...
def get_med(idx, med_input_data, input_t_time, pre_day, map_data, all_sample):
    ...
    for m in range(len(med_input_data)):
        try:
            ...
            temp = med_input_data[m][1]  # 找到服用药记录 [ [a, b] [c, d] ]  a,c代表是否服用，b,d代表距离爆发点之前的天数
            temp1 = np.asarray(temp)
            temp2 = temp1[:, -1]  # [b, d]
