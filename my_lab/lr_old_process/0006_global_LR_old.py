...
def save_weight_importance_to_csv(weight_important, max_iter):
    ...
    wi_file_name = os.path.join(MODEL_SAVE_PATH, f"0006_{pre_hour}h_global_lr_lbfgs_{max_iter}.csv")
...
def global_train(max_iter):
    ...
    train_x_ft = ss.fit_transform(train_x)
    test_x_ft = ss.fit_transform(test_x)
    # train_x_ft = train_x
    # test_x_ft = test_x

    lr_all = LogisticRegression(max_iter=max_iter, n_jobs=1)
    ...
    # # save model
    # model_file_name = os.path.join(MODEL_SAVE_PATH, f"{pre_hour}h_global_lr_lbfgs_{max_iter}.pkl")
    # pickle.dump(lr_all, open(model_file_name, "wb"))
    # my_logger.info(f"save lr model to pkl - [{model_file_name}]")

    my_logger.info(f'n_jobs:1, max_iter:{max_iter}, cost time: {run_time} s, auc: {auc}')
...
def sub_global_train(max_iter, x_train, y_train):
    """
    选取10%的数据进行训练
    :param train_x:
    :param train_y:
    ...
    """
    ss = StandardScaler(with_std=True, with_mean=True)
    train_x_ft = ss.fit_transform(x_train)
    test_x_ft = ss.fit_transform(test_x)
    # train_x_ft = x_train
    # test_x_ft = test_x

    lr_all = LogisticRegression(max_iter=max_iter)
    lr_all.fit(train_x_ft, y_train)
    y_predict = lr_all.decision_function(test_x_ft)
    auc = roc_auc_score(test_y, y_predict)

    run_time = round(time.time() - start_time, 2)

    my_logger.info(f'max_iter:{max_iter}, train_iter:{lr_all.n_iter_}, cost time: {run_time} s, auc: {auc}')
