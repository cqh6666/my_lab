...
def sub_global_train(max_iter):
    ...
    select_rate = 0.1
    train_x_ft, test_x_ft = get_train_data_for_random_idx(train_x, train_y, select_rate)
    ...
    my_logger.info(f'[sub_global] - solver:liblinear, max_iter:{max_iter}, train_iter:{lr_all.n_iter_}, cost time: {run_time} s, auc: {auc}')
