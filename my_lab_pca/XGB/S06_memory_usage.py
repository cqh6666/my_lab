...
def personalized_modeling(test_id, pre_data_select, pca_pre_data_select):
    ...
    try:
        ...
        similar_rank['distance'] = abs((train_data_x - pca_pre_data_select.values)).sum(axis=1)
    ...
    my_logger.warning(f"{fit_train_x.shape}, {fit_train_y.shape}")
    try:
...
if __name__ == '__main__':
    ...
    with ThreadPoolExecutor(max_workers=pool_nums) as executor:
        ...
        for test_id in test_id_list:
            ...
            pca_pre_data_select = mean_test_data_x.loc[[test_id]]
