...
if iter_init == 0:
    os.system(f'sbatch /panfs/pfs.local/work/liu/xzhang_sta/chenqinhai/code_pca/code_xgb/S06_learn_psm_with_pca.sh {is_transfer} {iter_init}')
    iter_init += step
...
while iter_init < 121:
    ...
    if os.path.exists(wi_file_name):
        time.sleep(5)
        os.system(
            f'sbatch /panfs/pfs.local/work/liu/xzhang_sta/chenqinhai/code_pca/code_xgb/S06_learn_psm_with_pca.sh {is_transfer} {iter_init}')
